...
int main(){
	...
	fft(n,20,&xr,&xi);
	double* Xmag = (double*)malloc(n * sizeof(double));
	for(int i=0;i<n;i++){
        double temp;
        temp = xr[i]*xr[i] + xi[i]*xi[i];
        Xmag[i] = sqrt(temp);
    }
    fout1  = fopen("../Data/Xnfft.dat", "w");
    for(int i = 0; i < n; i++){
        fprintf(fout1, "%lf+%lfi \n", xr[i],xi[i]);
    }
    fft(n,20,&hr,&hi);
    
    double* Hmag = (double*)malloc(n * sizeof(double));
	for(int i=0;i<n;i++){
        double temp1;
        temp1 = hr[i]*hr[i] + hi[i]*hi[i];
        Hmag[i] = sqrt(temp1);
    }
    fout2  = fopen("../Data/Hnfft.dat", "w");
    for(int i = 0; i < n; i++){
